...
// @route   GET api/posts
// @desc    Get posts for logged in user or the user whose home page one is browsing
// @access  Public
router.get(
  "/",
  passport.authenticate("jwt", { session: false }),
  (req, res) => {
    var id = req.params.curr_user;
    if (id == null) {
      id = req.user.id;
    }
...
router.get(
  ...
  (req, res) => {
    ...
    Profile.findOne({ user: req.user.id }).then(profile => {
      console.log("Hi " + profile.following);
      var followingList = profile.following;
      var loggedUser = {user: req.user.id};
      followingList.push(loggedUser);
      var results;

      Promise.all(followingList.map(getUserPosts))
        .then(posts => {
          // console.log("Posts: " + JSON.stringify(posts, null, 4));
          results = Array.prototype.concat.apply([], posts);
        })
        .catch(err => res.status(404).json({ nopostsfound: "No posts found" }))
